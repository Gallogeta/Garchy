...
def step_install(disk, username, password, hostname):
    ...
    run("pacstrap -K /mnt base base-devel linux linux-firmware sudo git zsh starship hyprland waybar rofi dunst kitty thunar pipewire pipewire-pulse wireplumber networkmanager ttf-jetbrains-mono-nerd noto-fonts-emoji efibootmgr snapper btrfs-progs python jq fastfetch")
    ...
    run(f"cp /usr/bin/garchy-ai /mnt/usr/bin/garchy-ai 2>/dev/null || true")
    run(f"cp /usr/bin/garchy-update /mnt/usr/bin/garchy-update 2>/dev/null || true")
    run(f"ln -sf /usr/bin/garchy-ai /mnt/usr/bin/ai")
    run(f"chmod +x /mnt/usr/bin/garchy-ai /mnt/usr/bin/garchy-update 2>/dev/null || true")
    ...
    print(f"\n{BOLD}[7/7] Enabling Core Services...{RESET}")
