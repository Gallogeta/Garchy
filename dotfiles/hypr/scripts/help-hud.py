...
Clean, friendly cheatsheet for all users, synchronized with the active Gally theme in real time.
"""
...
class HelpHudApp(tk.Tk):
    def __init__(self):
        ...
        # Header
        self.hdr = tk.Frame(self, bg=self.bg_main, padx=20, pady=15)
        self.hdr.pack(fill="x")
        
        self.lbl_title = tk.Label(self.hdr, text="🌌 Garchy OS — Quick Guide & Shortcuts", font=("Sans", 16, "bold"),
                                  fg=self.accent_gold, bg=self.bg_main)
        self.lbl_title.pack(anchor="w")
        self.lbl_sub = tk.Label(self.hdr, text="Everything you need to know to navigate your system easily", font=("Sans", 10),
                                fg=self.accent_cyan, bg=self.bg_main)
        self.lbl_sub.pack(anchor="w")
        self.sep = tk.Frame(self.hdr, height=2, bg=self.accent_gold)
        self.sep.pack(fill="x", pady=(8, 0))
        
        # Cards Grid Container
        self.container = tk.Frame(self, bg=self.bg_main, padx=20, pady=10)
        self.container.pack(fill="both", expand=True)
        
        self.render_cards()
        
        # Footer
        self.footer = tk.Frame(self, bg=self.bg_main, padx=20, pady=12)
        self.footer.pack(fill="x", side="bottom")
        
        self.lbl_tip = tk.Label(self.footer, text="💡 Tip: Press [ F1 ] anytime to open or close this helper card.",
                                font=("Sans", 9, "italic"), fg=self.fg_muted, bg=self.bg_main)
        self.lbl_tip.pack(side="left")
        
        self.btn_close = tk.Button(self.footer, text="Got it! (Close)", font=("Sans", 10, "bold"),
    ...
    def apply_theme_live(self):
        ...
        self.lbl_title.configure(fg=self.accent_gold, bg=self.bg_main)
        self.lbl_sub.configure(fg=self.accent_cyan, bg=self.bg_main)
        self.sep.configure(bg=self.accent_gold)
        self.container.configure(bg=self.bg_main)
        self.footer.configure(bg=self.bg_main)
        self.lbl_tip.configure(fg=self.fg_muted, bg=self.bg_main)
        self.btn_close.configure(bg=self.bg_input, activebackground=self.accent_cyan)
        self.render_cards()
    ...
    def render_cards(self):
        ...
        for idx, (title, desc) in enumerate(cards):
            ...
            card = tk.Frame(self.container, bg=self.bg_card, padx=15, pady=12, relief="flat",
